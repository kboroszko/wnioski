...
export function runSolver(facility, doctors) {
  ...
  for (const quota of (facility.hourQuotas || [])) {
    ...
    const hasMax = quota.maxHoursPerWeek > 0;
    ...
    if (hasMax && totalHrs > quota.maxHoursPerWeek) {
      errors.push(`Specjalizacja „${quota.specialty}" ma przydzielone ${totalHrs.toFixed(1)}h, co przekracza limit ${quota.maxHoursPerWeek}h`);
    }
  }
